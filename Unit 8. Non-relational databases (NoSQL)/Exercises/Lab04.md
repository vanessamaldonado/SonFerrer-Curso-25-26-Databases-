...
db.clientes.deleteOne(
  { 
    "preferencias.categorias_favoritas": { $size: 0 } 
  })
...
//3 Eliminar todos los clientes sin teléfono
db.clientes.deleteMany({ telefono: { $exists: false } })
...
db.clientes.deleteMany(
  { 
    ciudad: { $in: ["Guadalajara", "Querétaro"] } 
  })
...
db.clientes.deleteMany(
  { 
    ciudad: "Querétaro", estatus: "activo" 
  })
...
db.clientes.deleteOne({ 
  $and: [
    { 
      metodos_pago: 
        { $elemMatch: 
           { tipo: "tarjeta_credito", numero: "3456" } 
        } 
    }
  ]
})
